...
export default function CollBrowserNav({ coll, collViewPath, totalFiles, selectMode, selectedFileCount, onLeave, onPathNav, onSaveToNode, onSelectToggle, onBatchSave }) {
  return (
    <div className="flex flex-col px-3 py-2 border-b border-white/[0.04] shrink-0 gap-1 bg-surface-card sticky top-0 z-10">
      ...
      <div className="flex items-center gap-2 text-[10px]">
        ...
        {!collViewPath && (
          <button onClick={onSaveToNode}
            className="text-[10px] bg-brand-600 hover:bg-brand-700 px-2 py-0.5 rounded">💾 保存到本机</button>
        )}
        <button onClick={onSelectToggle} className={`text-[10px] px-2 py-0.5 rounded ${selectMode ? 'bg-brand-600 text-white' : 'bg-white/[0.06] text-gray-400'}`}>
          {selectMode ? '取消' : '选择文件'}
        </button>
      </div>
      {selectMode && selectedFileCount > 0 && (
        <button onClick={onBatchSave}
          className="text-[10px] bg-brand-600 hover:bg-brand-700 px-2 py-1 rounded">
