...
export default function FileSourceRow({ file, onAdd, onDragStart, onSelect }) {
  ...
  return (
    <div draggable onDragStart={onDragStart ? (e) => onDragStart(e, file) : undefined}
      onClick={handleClick}
      className="flex items-center gap-2 px-3 py-2 hover:bg-white/[0.06] border-b border-white/[0.03] text-sm group cursor-pointer">
      <a href="#" onClick={e => {
        e.preventDefault();
        e.stopPropagation();
      ...
      <span className="text-brand-300 truncate flex-1 font-mono text-[11px]">{file.filename}</span>
      ...
      <button onClick={(e) => { e.stopPropagation(); onAdd(file.hash, file.filename, file.mime_type, file.size); }}
        className="text-brand-400 opacity-0 group-hover:opacity-100 text-sm px-2 py-1 rounded bg-brand-600/20 hover:bg-brand-600/40 shrink-0">+</button>
    </div>
  );
}
