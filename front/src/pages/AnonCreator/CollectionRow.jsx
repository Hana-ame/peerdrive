...
export default function CollectionRow({ collection, selectMode, isSelected, onClick, onToggleSelect }) {
  ...
  return (
    <div className={`px-4 py-3 border-b border-white/[0.03] cursor-pointer hover:bg-white/[0.06] ${isSelected ? 'bg-brand-900/30' : ''}`}
      onClick={handleClick}>
      <div className="flex items-center gap-2">
        {selectMode && <input type="checkbox" checked={isSelected} readOnly className="shrink-0" />}
        <span className="text-lg">📦</span>
        <span className="text-brand-300 truncate flex-1 text-sm font-medium">{collDisplayName(collection)}</span>
      </div>
      <div className="flex items-center gap-2 mt-1 ml-8">
        {collection.tags?.map(t => <span key={t} className="text-[10px] bg-brand-900/50 text-brand-300 px-1.5 py-0.5 rounded-full">{t}</span>)}
