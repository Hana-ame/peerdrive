      ...
      {sysLoading ? (
        ...
        sysEntries.map(e => (
          <div key={e.path} draggable={!e.is_dir}
            ...
            className={`flex items-center gap-3 px-4 py-2.5 hover:bg-white/[0.06] border-b border-white/[0.03] text-sm group ${e.is_dir ? 'cursor-pointer' : ''}`}>
            <span className="text-lg">{e.is_dir ? '📁' : '📄'}</span>
            <span className={`font-mono truncate flex-1 text-xs ${e.is_dir ? 'text-yellow-400' : 'text-brand-300'}`}>{e.name}</span>
            {e.is_dir ? (
              ...
              <button onClick={(ev) => { ev.stopPropagation(); onAddFile(e.path, e.name, e.size || 0); }}
                className="text-brand-400 opacity-60 group-hover:opacity-100 text-sm px-2 py-1 rounded bg-brand-600/20 hover:bg-brand-600/40 shrink-0">+</button>
            )}
          </div>
        ))
      )}
