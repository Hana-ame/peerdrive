...
function Avatar({ name }) {
  ...
  return (
    <span className="w-7 h-7 shrink-0 rounded-full bg-blue-600/30 text-blue-200 flex items-center justify-center text-xs">
      {ch}
    </span>
  );
}
...
export default function AccountPicker({ accounts = [], selected = [], onConfirm, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div className="bg-gray-800 rounded-xl border border-gray-600 shadow-2xl w-[380px] max-h-[80vh] flex flex-col"
        ...
        <div className="p-3 space-y-2 border-b border-gray-700">
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="搜索昵称 / @id..."
            className="w-full bg-gray-900 text-xs px-2 py-1.5 rounded border border-gray-700 focus:outline-none focus:border-blue-600" />
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          ...
          {filtered.map(a => (
            <div key={a.username} onClick={() => toggle(a.username)}
              ...
              <span className={`text-xs ${picked.has(a.username) ? 'text-blue-400' : 'text-gray-600'}`}>
        ...
        <div className="p-3 border-t border-gray-700 space-y-2">
          <div className="flex items-center gap-1">
            <input value={manual} onChange={e => setManual(e.target.value)}
              placeholder="手动添加 @id"
              className="flex-1 bg-gray-900 text-xs px-2 py-1.5 rounded border border-gray-700 focus:outline-none focus:border-blue-600 font-mono" />
            <button onClick={addManual} disabled={!manual.trim()}
              className="text-xs px-2 py-1.5 rounded bg-gray-700 hover:bg-gray-600 disabled:opacity-40">添加</button>
          </div>
          <button onClick={() => onConfirm(Array.from(picked))}
            className="w-full text-xs bg-blue-600 hover:bg-blue-700 py-1.5 rounded">
            确定（{picked.size} 人）
          </button>
        </div>
      </div>
