...
export default function FileManager() {
  ...
  // 数据目录当前层内容。必须在组件顶层调用 useMemo（不能在 renderDbDirBrowser 内调用，
  // 否则 hook 数量随浏览模式变化，React 抛 "Rendered more hooks than during the previous render"）
  const dbDirContents = useMemo(() => getDbDirContents(files, dbDirPath), [files, dbDirPath]);
  ...
  // 分享文件：复制下载链接到剪贴板（此前 handleShare 被引用却从未定义，点击会 ReferenceError 崩溃）
  const handleShare = async (file, e) => {
    e?.stopPropagation?.();
    const url = file.hash ? api.getDownloadUrl(file.hash) : '';
    if (!url) { setNotification('该文件没有可分享的下载链接'); return; }
    try {
      // 优先用 Clipboard API；不支持时退回 execCommand 兜底（本地 http 非 HTTPS 环境可能无 Clipboard API）
      ...
      setNotification(`已复制下载链接: ${url}`);
    } catch (err) {
      console.error('share copy failed:', err);
      setNotification('复制链接失败');
    }
  };
  ...
  const renderDbDirBrowser = () => {
    const { subdirs, files: dbFiles } = dbDirContents;
