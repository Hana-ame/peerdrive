...
func (b *BTBridge) ShareFile(hash string) error {
	...
	if b.DHT == nil {
		...
		LogError("bt-bridge: ShareFile failed: %v", err)
		return err
	}
	if err := b.DHT.Announce(hash); err != nil {
		LogError("bt-bridge: ShareFile announce failed: %v", err)
		return fmt.Errorf("share announce: %w", err)
	}
	b.mu.Lock()
	b.shared[hash] = struct{}{}
	b.mu.Unlock()
	LogInfo("bt-bridge: shared file %s on BT DHT", hash)
	return nil
}
...
func (b *BTBridge) FetchFile(ctx context.Context, hash string) ([]byte, error) {
	...
	if b.DHT == nil {
		err := fmt.Errorf("BT DHT not available")
		LogError("bt-bridge: FetchFile failed: %v", err)
		return nil, err
	}

	peers, err := b.DHT.FindProviders(hash)
	if err != nil {
		LogError("bt-bridge: FetchFile find providers failed: %v", err)
		return nil, fmt.Errorf("find providers: %w", err)
	}
	if len(peers) == 0 {
		LogInfo("bt-bridge: no BT DHT providers found for %s", hash)
		return nil, fmt.Errorf("no BT DHT providers found for %s", hash)
	}

	LogInfo("bt-bridge: found %d BT DHT providers for %s, attempting fetch", len(peers), hash)
	...
	for _, peerAddr := range peers {
		...
		if err != nil {
			LogWarn("bt-bridge: HTTP fetch from %s failed: %v", peerAddr, err)
			continue
		}
		...
		resp.Body.Close()
		if err != nil {
			LogWarn("bt-bridge: HTTP read from %s failed: %v", peerAddr, err)
			continue
		}

		if resp.StatusCode != http.StatusOK {
			LogWarn("bt-bridge: HTTP %d from %s", resp.StatusCode, peerAddr)
			continue
		}

		LogInfo("bt-bridge: fetched %s from %s (%d bytes)", hash, peerAddr, len(data))
		return data, nil
	}

	err = fmt.Errorf("could not fetch %s from any BT DHT peer", hash)
	LogError("bt-bridge: %v", err)
	return nil, err
}
