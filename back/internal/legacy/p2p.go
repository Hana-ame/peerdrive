...
const (
	...
	// maxP2PListBytes / maxP2PFileBytes 远端声明长度上限（H4 修复）：
	// OK/文件大小来自 DHT 公网任意节点，无上限校验 → make/Truncate 直接 OOM
	maxP2PListBytes = 256 * 1024 * 1024   // 集合列表（JSON）上限 256MB
	maxP2PFileBytes = 8 * 1024 * 1024 * 1024 // 文件内容上限 8GB（与上传一致）
...
func (p *P2PService) FindProviders(hash string) ([]peer.AddrInfo, error) {
	...
	// 低危 5 修复：快照后解锁再 Connect——之前持 RLock 做 5s 阻塞 Connect，
	// 期间所有读锁操作（count/iter）全部排队等锁
	p.mu.RLock()
	discovered := make([]peer.AddrInfo, 0, len(p.discovered))
	for _, info := range p.discovered {
...
func (p *P2PService) RequestCollectionList(ctx context.Context, peerID peer.ID, query string) ([]byte, error) {
	...
	// H4 修复：远端声明长度先过上限再分配——恶意节点回 `OK 4294967296`
	// → make 一次分配 4GB 直接 OOM（集合列表数据远小于 256MB 上限）
	if int64(size) > maxP2PListBytes {
		return nil, fmt.Errorf("peer declared oversized list %d (limit %d)", size, maxP2PListBytes)
	}
...
func (p *P2PService) requestData(ctx context.Context, peerID peer.ID, hash string) ([]byte, error) {
	...
	// H4 修复：与 RequestCollectionList 同一上限——恶意 `OK 1<<62` → OOM
	if int64(status) > maxP2PFileBytes {
		return nil, fmt.Errorf("peer declared oversized file %d (limit %d)", status, maxP2PFileBytes)
	}
...
func (p *P2PService) handleExchange(stream network.Stream) {
	...
	// M4 修复：流式发送——之前 os.ReadFile 整文件驻留内存（8GB 文件 = 8GB RAM，
	// 并发请求叠加 OOM）。改 open + stat + io.Copy 流式回传。
	filePath := filepath.Join(p.storageDir, hash[:2], hash)
	...
	info, err := f.Stat()
	if err != nil {
		log.LogDebug("p2p: handleExchange stat failed for %s", hash)
		stream.SetWriteDeadline(time.Now().Add(FileReadTimeout))
		fmt.Fprintf(stream, "ERR not found\n")
		return
	}

	log.LogInfo("p2p: handleExchange serving %s to %s (%d bytes)", hash, stream.Conn().RemotePeer().String(), info.Size())
	stream.SetWriteDeadline(time.Now().Add(FileReadTimeout))
	fmt.Fprintf(stream, "OK %d\n", info.Size())
	n, err := io.Copy(stream, f)
	if err != nil {
		log.LogWarn("p2p: handleExchange write to %s failed: %v", stream.Conn().RemotePeer().String(), err)
		return
	}

	if p.tracker != nil {
		p.tracker.RecordBytesSent(stream.Conn().RemotePeer().String(), n)
	}
}
...
func (p *P2PService) processWSRequests() {
	for req := range p.requestCh {
		...
		if req.ReplyCh != nil {
			// 低危 6 修复：select+default 静默丢数据——改阻塞+超时，
			// 慢消费者不丢（当前无调用方设置 ReplyCh，留着是地雷，复用时必踩）
			select {
			case req.ReplyCh <- resp:
			case <-time.After(5 * time.Second):
				log.LogWarn("p2p: reply channel full, dropping response for %s", req.Hash)
			}
		}
	}
}
