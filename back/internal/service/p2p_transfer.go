...
func (ct *ChunkedTransfer) DownloadFile(ctx context.Context, hash string, targetPath string, onProgress func(float64)) (*TransferProgress, error) {
	...
	// M3 修复：totalSize 由远端 SIZE 响应直接算出，恶意声明超大值 →
	// make(chan int, ChunksTotal) 分配 OOM + Truncate 稀疏超大文件。
	// requestFileSize 已校验，这里再兜一道（多 provider 场景可能路径不一致）
	if totalSize <= 0 || totalSize > maxP2PFileBytes {
		log.LogError("p2p-transfer: peer declared invalid file size %d", totalSize)
		return nil, fmt.Errorf("peer declared invalid file size %d", totalSize)
	}
...
func (ct *ChunkedTransfer) requestFileSize(ctx context.Context, peerID peer.ID, hash string) (int64, error) {
	...
	// M2 修复：Fscanf 前加读超时——之前无 deadline，对端不回包直接挂死
	// （同一文件里 requestChunk 有 deadline 唯独这里漏了）
	...
	// M3 修复：远端声明的文件大小先过上限再返回——恶意 SIZE 1<<62 会在
	// DownloadFile 里 make(chan)+Truncate 直接 OOM/稀疏超大文件
	if size <= 0 || size > maxP2PFileBytes {
		return 0, fmt.Errorf("peer declared invalid size %d", size)
	}
	return size, nil
}
