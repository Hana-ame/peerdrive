// node_share.go：本节点共享范围的管理端点（doc/NETDISK.md M2.6）。
//
// 语义：回答并修改「我这个节点对外提供什么」：
//   - GET  /peerjs/share         当前共享范围 + 可选文件清单（带是否已共享）
//   - PUT  /peerjs/share         局部更新（enable/dirs/files/collections）
//   - POST /peerjs/share/files   勾选/取消若干文件 {hashes:[], shared:bool}
//
// 为什么要这套端点：共享范围原本只能靠 PEERDRIVE_SHARE_* 环境变量在启动时定，
// 想改就得重启节点。而"我愿意把哪些文件给出去"是随手的决定——新上传一个文件
// 想立刻共享、某个目录不想给了。要求重启等于逼运营者要么长期共享一个过宽的
// 目录，要么干脆不开共享（见 service/nodeshare.go 文件头）。
//
...
// 返回当前共享范围 + 可选文件清单（每行带 shared/by_dir），管理台据此渲染勾选框。
...
func PutNodeShare(c *gin.Context) {
	...
	if err != nil {
		// 校验失败（卷根目录 / 非法 hash）是用户输入问题，400 而不是 500
...
// PostNodeShareFiles 处理 POST /peerjs/share/files {hashes:[...], shared:bool}。
//
...
func PostNodeShareFiles(c *gin.Context) {
	...
	scope, err := nodeShareSvc.SetFilesShared(body.Hashes, body.Shared)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
