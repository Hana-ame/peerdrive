...
| 你的情况 | 做法 | 看哪一节 |
|---|---|---|
| 自己跑一个节点，在本机或局域网里连它 | 下载二进制 + 自托管信令 | §1.1 → §1.2 → §1.3 → §1.5 |
| 想从另一台电脑 / 手机（外网）连自己的节点 | 下载二进制 + 线上信令（wss） | §1.1 → §1.3 → §1.6 |
...
| 机器 | 节点（server） | 自托管信令（signaling） |
|---|---|---|
| Linux x86_64 | `peerdrive-linux-amd64` | `peersignal-linux-amd64` |
| Linux ARM64（树莓派 / 服务器） | `peerdrive-linux-arm64` | `peersignal-linux-arm64` |
| Windows x86_64 | `peerdrive-windows-amd64.exe` | `peersignal-windows-amd64.exe` |
| macOS Intel | `peerdrive-darwin-amd64` | `peersignal-darwin-amd64` |
| macOS Apple Silicon | `peerdrive-darwin-arm64` | `peersignal-darwin-arm64` |

> **两个都要下**：`peerdrive-*` 是节点本体，`peersignal-*` 是自托管信令。
> 如果你想直接连线上现成的信令（§1.6），信令那个可以不下。
...
gh release download --repo Hana-ame/peerdrive --pattern 'peersignal-linux-amd64'

# 只有 curl
curl -LO https://github.com/Hana-ame/peerdrive/releases/latest/download/peerdrive-linux-amd64
curl -LO https://github.com/Hana-ame/peerdrive/releases/latest/download/peersignal-linux-amd64
```

下完的收尾动作：

```bash
chmod +x peerdrive-linux-amd64 peersignal-linux-amd64     # Linux / macOS
```
...
信令只负责转发 SDP/ICE 帮两端打洞，**不碰数据面** —— 文件内容走 WebRTC DataChannel，不经过它。
本机 / 局域网玩的时候自己起一个最省事，也不依赖外网。

```bash
./peersignal-linux-amd64 -addr :9100 -key peerjs
# 探活
...
|---|---|
| `-addr` | 监听地址，默认 `:9000` |
| `-key` | 命名空间。**节点侧的 `PEERDRIVE_PEERJS_KEY` 必须与它一致**，否则连不上 |
| `-tokens` | 信令 token 白名单（逗号分隔，空 = 不限制） |
| `-tls-cert` / `-tls-key` | **必须成对给**才切到 HTTPS/WSS；只给一个会报错退出（§1.6） |
...
| `PEERDRIVE_PEERJS_HOST/PORT/KEY/SECURE` | 信令地址与命名空间，必须和 §1.2 一致 |
...
| wss 勾选框 | `in-secure` | **HTTPS 页面必须勾**（§1.5.3）；本地 `ws://` 可取消 |
...
> 本仓库的自托管信令已放开 CORS；**线上的 `peersignal.moonchan.xyz` 实测没有这个头**
> （2026-09-22 复验），所以在那条路上只能用「手动填 id」。
...
```bash
./peersignal-linux-amd64 -addr :9100 -key peerjs -tls-cert cert.pem -tls-key key.pem
```

两个 TLS 参数必须**同时给**，只给一个会直接报错退出（这是故意的：静默降级成 ws 更危险）。
不想改程序就挂一层 TLS 反代（nginx / Caddy）也行。

> 注意：信令实现方式不限，任何 PeerJS 兼容信令都可以（公共 PeerJS 云、
> Node.js `peerjs-server`、本仓库的自托管信令）—— 只要节点和面板指向同一个、`key` 一致即可。
...
> 想留一份本地的单文件版，见第二章的 `npm run build:panel`。
...
| 症状 | 真因 | 处置 |
|---|---|---|
| 面板只说"连不上"，没别的信息 | HTTPS 页面 + `ws://` 信令被混合内容拦掉 | 勾 wss / 信令开 TLS（§1.6） |
