...
> 第一章是「下载 release 直接用」，这一章是**什么时候非自己编不可**：
> 改了代码、要跑还没发版的提交、要跑测试与端到端、或要自己出包。
> 只是想跑起来的话，回到[第一章](01-run-and-connect.md)就行。
...
| 用面板连节点 | ❌ | 在线面板，或第一章的链接 |
| 改 Go 代码并验证 | ✅ | 本章 §2.2 / §2.3 |
| 跑最新未发版的 `refactor` | ✅ | 本章 |
| 跑端到端验收（市场/加入/拉取/门禁） | ✅ | §2.4 的一键脚本 |
| 自己出一份发布包 | ✅ | §2.8（打 `v*` tag 让 CI 编） |
