...
import FileTree, { buildFlatTree } from '../components/FileTree';
...
export default function AnonCreator() {
  ...
  const loadFiles = async () => {
    setFLoading(true);
    try { setFiles(await api.listFiles(sort) || []); }
    catch { setFiles([]); }
    setFLoading(false);
  };
  ...
  const collFiltered = collSource?.entries?.filter(e => {
    if (search && !(e.path || '').toLowerCase().includes(search.toLowerCase())) return false;
    return true;
  }) || [];
  ...
  const onSplitMouseDown = (e) => {
    ...
    const onMove = (ev) => {
      ...
      const pct = Math.max(20, Math.min(80, startSplit + (dx / window.innerWidth) * 100));
      setSplit(pct);
    };
  ...
  return (
    <div className={`flex flex-1 overflow-hidden h-full bg-gray-950 ${dragging ? 'select-none' : ''}`}>
      ...
      <div style={{ width: `${split}%` }} className="h-full flex flex-col border-r border-gray-700">
        <div className="h-10 flex items-center px-3 border-b border-gray-800 shrink-0 gap-1">
          <div className="flex bg-gray-800 rounded">
            <button onClick={() => setSrcTab('local')} className={`px-3 py-1 text-[11px] rounded ${srcTab === 'local' ? 'bg-gray-600 text-white' : 'text-gray-400'}`}>本地文件</button>
            <button onClick={() => setSrcTab('collection')} className={`px-3 py-1 text-[11px] rounded ${srcTab === 'collection' ? 'bg-gray-600 text-white' : 'text-gray-400'}`}>已有合集</button>
          </div>
          <div className="flex-1" />
          <span className="text-[10px] text-gray-600">{srcTab === 'local' ? filtered.length : collFiltered.length} 项</span>
        </div>

        {srcTab === 'collection' && (
          <div className="p-2 border-b border-gray-800 shrink-0 space-y-1 max-h-[200px] overflow-y-auto">
            {collections.length === 0 ? (
              <p className="text-[10px] text-gray-600 p-2">暂无历史合集</p>
            ) : collections.map(c => (
              <div key={c.hash} onClick={() => loadCollAsSource(c.hash)}
                className={`text-[10px] px-2 py-1 rounded cursor-pointer hover:bg-gray-800 flex items-center justify-between ${collSource && c.hash === openHash ? 'bg-blue-900/30' : ''}`}>
              ...
              <div key={f.hash} draggable
                ...
                {!inDraft(f.filename, f.hash) && (
                  <button onClick={() => addEntry(f.hash, f.filename, f.mime_type, f.size)}
                    className="text-blue-400 hover:text-blue-200 opacity-0 group-hover:opacity-100 text-[10px] px-1.5 py-0.5 rounded bg-blue-600/20 hover:bg-blue-600/40 transition-all shrink-0">
                    +
                  </button>
                )}
              </div>
      ...
      <div style={{ width: `${100 - split}%` }} className="h-full flex flex-col">
        <div className="h-12 flex items-center px-4 space-x-3 border-b border-gray-800 shrink-0">
          <Link to="/" className="text-xs text-gray-500 hover:text-white shrink-0">←</Link>
          <input value={fname} onChange={e => setFname(e.target.value)} placeholder="合集名称 (可选)" className="bg-gray-800 text-xs px-2 py-1 rounded border border-gray-700 w-40 focus:outline-none focus:border-blue-500" />
          <div className="flex-1" />
          {entries.length > 0 && !savedHash && (
            <span className="text-[10px] text-yellow-500 bg-yellow-500/10 px-2 py-0.5 rounded border border-yellow-600/30">未保存</span>
          )}
          {savedHash && entries.length > 0 && (
            <span className="text-[10px] text-yellow-500 bg-yellow-500/10 px-2 py-0.5 rounded border border-yellow-600/30">已修改</span>
          )}
          <button onClick={handleMint} disabled={saving || !entries.filter(e => e.path && e.hash).length}
            className="bg-green-600 hover:bg-green-700 disabled:opacity-40 text-xs px-3 py-1.5 rounded font-medium">
            Mint
          </button>
          {savedHash && (
            <button onClick={handleCommit} disabled={saving}
              className="bg-blue-600 hover:bg-blue-700 disabled:opacity-40 text-xs px-3 py-1.5 rounded font-medium">
              Commit
            </button>
          )}
        </div>
