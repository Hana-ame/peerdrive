...
export default function VersionLog({ username, collName, triggerRefresh }) {
  ...
  const loadLog = async () => {
    try { const res = await getVersionLog(username, collName); setVersions(res.data || []); }
  ...
  return (
    <div className="p-4">
      ...
      <div className="space-y-3">
        {versions.length === 0 ? <p className="text-gray-500 text-sm">暂无历史</p> : (
          versions.map((v, idx) => (
            <div key={v.id} className="bg-gray-700/50 p-3 rounded border border-gray-600 relative group hover:border-blue-500 transition-colors">
              ...
              <p className="text-sm text-white mb-2 truncate">{v.commit_message || 'No message'}</p>
              {idx > 0 && (
                <button
